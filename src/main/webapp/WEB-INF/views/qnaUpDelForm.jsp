	...
	$(document).ready(function(){
		...
		function checkQnaUpDelForm(upDel) {
			...
			if(upDel == "del") {
				...
				document.qnaUpDelForm.upDel.value="del";
				//--------------------------------------------------------------------------------------------------------
			...
			//----------------------------------------------------------------------
			// 현재 화면에서 페이지 이동 없이 서버쪽 "support/qnaUpDelProc.do" 를 호출
			// 게시판 수정 또는 삭제 적용 개수 가 있는 html 소스를  받기
			$.ajax({
				// 호출할 서버쪽 URL 주소 설정
				url : "/support/qnaUpDelProc.do"
				
				// 전송방법 설정
				, type : "post"
				
				// 서버에 보낼 파라미터값 설정
				, data : $("[name=qnaUpDelForm]").serialize()
				
				, success : function( upDelCnt ) {	
				
					// 게시판 새글 입력 행 적용개수 가 한개면 메세지 띄우고 /support/qnaListForm.do 로 이동
				if(upDel == "up") {
					if(upDelCnt == 1) {
					alert("수정성공!");
